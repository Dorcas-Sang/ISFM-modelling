...
ISFM_cdf_data <- ISFM_cdf_data %>%
  mutate(
    Outcome_label = case_when(
      ...
      Outcome == "income" ~ "b. Maize and soybean income (GHS/ha)",
      Outcome == "environmental" ~ "c. Environmental benefits (GHS/ha)",
      Outcome == "social" ~ "d. Social benefits (GHS/ha)",
      Outcome == "human" ~ "e. Human benefits (GHS/ha)",
      TRUE ~ Outcome
    ),
    Outcome_label = factor(Outcome_label, levels = c(
      "a. Maize productivity (kg/ha)",
      "b. Maize and soybean income (GHS/ha)",
      "c. Environmental benefits (GHS/ha)",
      "d. Social benefits (GHS/ha)",
      "e. Human benefits (GHS/ha)"
    )),
...
# Extract data from the mcSimulation object
ISFM_data_box <- as.data.frame(ISFM_mc_simulation$y)[, ordered_vars]

# Long format + labeled, ordered factor
ISFM_data_long <- pivot_longer(
  ISFM_data_box,
  cols = everything(),
  names_to = "Scenario",
  values_to = "NPV"
...
# Box plot
p <- ggplot(ISFM_data_long, aes(x = Scenario_label, y = NPV, fill = Scenario_label)) +
  geom_boxplot(
    width = 0.6,
    color = "black",
    outlier.alpha = 0.6,
...
# Save
ggsave("figures/NPV_ISFM_gender_resource_box.png", plot = p, width = 8, height = 5, dpi = 1000)

```


Baseline and ISFM components cumulative Cashflow projection
...
#Apply the new labels to the facet strips
# Only if the plot uses facet_wrap (most likely from plot_cashflow)
...
# 5. Force the facets to one column layout (optional, but usually desirable)
# Only if facet_wrap was used
if ("FacetWrap" %in% class(ISFM_components_cashflow_plot$facet)) {
  ISFM_components_cashflow_plot$facet$params$ncol <- 1
}
...
Cashflow analysis and plotting based on gender and farming resources (Not used in the paper)
...
ISFM_cashflow_men <- plot_cashflow(mcSimulation_object = ISFM_mc_simulation,
              cashflow_var_name = "men_cashflow_ISFM",
                          ...
                          color_5_95 = "yellow",
                          color_median= "red",
                          base= 10 ) + ggplot2::scale_y_continuous(limits = c(0, 2000000))
...
# Group VIP scores into Low/Medium/High
all_data <- all_data %>%
  mutate(
    vip_category = cut(
      VIP,
